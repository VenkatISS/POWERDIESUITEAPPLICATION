...
if(users_data.length>0) {
	for(var z=0; z<users_data.length; z++) {
		if(users_data[z].deleted == 0){
		staffdatahtml=staffdatahtml+"<OPTION VALUE='"+users_data[z].userId+"'>"+users_data[z].userName+"</OPTION>";
		}	
	}
}
		...
		for(var i=page_data[z].quotationItemsList.length-1; i>=0; i--){
			...
			rowpdhtml = rowpdhtml + "<td>"+page_data[z].qtn_amount+"</td>";
				...
				rowpdhtml = rowpdhtml + "<td>"+spd+"</td>"+
						...
						"<td>"+page_data[z].quotationItemsList[i].foot_notes+"</td>"+
...
function addRow() {
	...
	if(ele.length<4){
		...
		l.innerHTML = "<td><img src='images/delete.png' onclick='doRowDeleteInTranxs(this,amt,qtn_c_amt,data_table_body)'></td>";
	...
	if(document.getElementById("epid") != null){
		...
		if(elements.length==1) {
			...
			for(var i=0; i<elements.length; i++) {
				...
				var fn = formobj.fn[i].value.trim();
...
function deleteItem(iid,invdate,detailsListlen){
//	var flag=validateDayEndForDelete(invdate,dedate);
	var flag=validateDayEndForDelete(invdate);
	...
	if(document.getElementById("bp") != null) {	
		...
		if(elements.length==1) {
			...
			if(pide.selectedIndex > 0) {
				...
				var gstp = fetchGSTPercent(product_data,pidv) != 0 ? fetchGSTPercent(product_data,pidv):"";
				var upv = fetchProductUnitPrice(product_prices_data,pidv) != 0 ? fetchProductUnitPrice(product_prices_data,pidv):"";	
				if(gstp>0 && upv>0) {
					for(var p=0; p<cat_prod_types_data.length; p++){
						var cproduct = cat_prod_types_data[p].cat_name+"-"+cat_prod_types_data[p].cat_desc;
						var prodid = cat_prod_types_data[p].id;
						
						for(var i=0;i<product_prices_data.length;i++){
					...
					document.getElementById("dialog-1").innerHTML = "Please ADD the PRODUCT in PRODUCT MASTER.";
					alertdialogue();
					//alert("Please ADD the PRODUCT in EQUIPMENT MASTER.");
					return;
				}else if(upv<=0){
					document.getElementById("dialog-1").innerHTML = "Please ADD PRODUCT'S price in PRODUCT PRICE MASTER.";
					alertdialogue();
