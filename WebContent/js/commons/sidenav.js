...
function fetchProductDetails(types, pc) {
	...
	for(var i=0; i< types.length; i++){
		if(types[i].id == pc) {
			return types[i].cat_name+"-"+types[i].cat_desc;
			//return getProductType(types[i].prod_code)+" - "+getRawMaterialType(types[i].rawmat_code);
		}
	}
}
...
function fetchProductUnitPrice(pc_data, pcid) {
...
function getProductType(prod_code){
	if(prod_code==1)
		return "HONOR 7X";
	if(prod_code==2)
		return "HONOR 8X";
...
function getRawMaterialType(rawmat_code){
	if(rawmat_code==11)
		return "CU";
	if(rawmat_code==12)
		return "SILVER";
